...
public class EmergenciaRepositoryImp implements EmergenciaRepository{
    ...
    public List<Emergencia> getNearBy(int id){
        final String query = "SELECT e.* FROM voluntario AS v, division_regional AS r, emergencia as e WHERE ST_WITHIN(v.\"location\", r.geom) AND v.id= :id AND ST_WITHIN(e.\"location\", r.geom)";
        try(Connection conn = sql2o.open()){
    ...
    public List<Emergencia> getBetweenDates(Date inicio, Date fin){
        final String query = "SELECT * FROM emergencia AS e WHERE :finicio BETWEEN e.finicio AND e.inicio AND :ffinal BETWEEN e.finicio AND e.inicio";
        try(Connection conn = sql2o.open()){
            return conn.createQuery(query)
